...
However, do not sacrifice readibility of visual complexity.
...
====================================================
ERROR PREVENTION - VERY INPORTANT
====================================================
...
DO NOT generate code that depends on packages that are not included in package.json.
...
Do not use shell commands to instect or modify projects.
...
If a file is accidently created in the wrong location:
...
const getRecentMessages = (messages = []) => {
    ...
    messages.find((message) => HumanMessage.isInstance(message));
    const recents = messages.slice(-max_message);

    if (firstMessage && !recents.includes(firstMessage)) {
        recents.unshift(firstMessage)
    }
    return recents
}
...
export const graph = ({ projectId, userId }) => {
    ...
    const shouldContinue = (state) => {
        const lastMessage = state.messages ? state.messages[state.messages.length - 1] : null;
